...
const mockReportesGenerados: ReporteGenerado[] = [
  {
    id: 1,
...
export const useReportes = () => {
  const [reportes, setReportes] = useState<ReporteGenerado[]>(mockReportesGenerados);
  ...
  const generarDatosValorizacion = useCallback((filtros: FiltrosReporte): DatosReporteValorizacion => {
  ...
  const generarDatosAvance = useCallback((filtros: FiltrosReporte): DatosReporteAvanceObra => {
    ...
    const recomendaciones: RecomendacionObra[] = [
      {
        id: '1',
  ...
  const generarDatosFinancieros = useCallback((): DatosReporteFinanciero => {
    return {
      resumenEjecutivo: {
  ...
  const generarDatosContractuales = useCallback((): DatosReporteContractual => {
    return {
      contratosVigentes: [
        {
          obraId: 1,
  ...
  const generarDatosGerenciales = useCallback((): DatosReporteGerencial => {
    return {
      kpisPrincipales: {
        inversionTotal: 15750000,
        avanceGeneral: 9.8,
  ...
  const generarReporte = useCallback(async (solicitud: SolicitudReporte): Promise<ReporteGenerado> => {
    setLoading(true);
    setError(null);
  ...
  const obtenerDatosReporte = useCallback((tipo: TipoReporte, filtros: FiltrosReporte) => {
    switch (tipo) {
  ...
  const eliminarReporte = useCallback(async (id: number): Promise<boolean> => {
    setLoading(true);
    try {
  ...
  const descargarReporte = useCallback(async (id: number): Promise<string | null> => {
    try {
  ...
  const configuracionesGraficos = useMemo<Record<string, ConfiguracionGrafico>>(() => ({
    avanceMensual: {
      id: 'avance-mensual',
      tipo: 'LINE',
...
export const useEstadisticasReportes = () => {
  ...
  const estadisticas = useMemo(() => {
    ...
    return {
      ...
      totalDescargas,
      reporteMasDescargado: reportes.reduce((prev, current) => 
        (prev.numeroDescargas > current.numeroDescargas) ? prev : current
      ),
      tiempoPromedioGeneracion: reportesCompletados.reduce((sum, r) => 
        sum + (r.tiempoGeneracion || 0), 0
      ) / (reportesCompletados.length || 1),
