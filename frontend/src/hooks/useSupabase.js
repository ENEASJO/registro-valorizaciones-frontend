...
export function useObras(filtros) {
    const [obras, setObras] = useState([]); // any[] porque incluye relaciones
    const [loading, setLoading] = useState(true);
    const [error, setError] = useState(null);
    const cargarObras = async () => {
        try {
            ...
            if (filtros?.search) {
                obrasFiltradas = obrasFiltradas.filter(obra => obra.nombre.toLowerCase().includes(filtros.search.toLowerCase()) ||
    ...
    const actualizarObra = async (id, obra) => {
        try {
...
export function useValorizaciones(filtros) {
    const [valorizaciones, setValorizaciones] = useState([]); // any[] porque incluye relaciones
    const [loading, setLoading] = useState(true);
    const [error, setError] = useState(null);
    ...
    const crearValorizacion = async (valorizacion) => {
        try {
    ...
    const actualizarValorizacion = async (id, valorizacion) => {
        try {
    ...
    const eliminarValorizacion = async (id) => {
        try {
