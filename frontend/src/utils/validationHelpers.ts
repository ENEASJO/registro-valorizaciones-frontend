...
export const DEFAULT_VALIDATION_CONFIG: ConfiguracionValidacion = {
  campos_requeridos: {
...
export const validarRUC = (ruc: string): boolean => {
  ...
  if (!['10', '20'].includes(rucLimpio.substring(0, 2))) return false;
 ...
 */
export const validarSeccionBasica = (
  formData: EmpresaFormConsolidado,
  ...
  camposRequeridos.forEach(campo => {
    const valor = formData[campo as keyof EmpresaFormConsolidado];
 ...
 */
export const validarSeccionContacto = (
  formData: EmpresaFormConsolidado,
  config: ConfiguracionValidacion
): ErrorValidacion[] => {
  const errores: ErrorValidacion[] = [];
 ...
 */
export const validarSeccionRepresentantes = (
  formData: EmpresaFormConsolidado,
  config: ConfiguracionValidacion
): ErrorValidacion[] => {
  const errores: ErrorValidacion[] = [];
 ...
 */
export const validarSeccionRegistro = (
  formData: EmpresaFormConsolidado,
  config: ConfiguracionValidacion
): ErrorValidacion[] => {
  const errores: ErrorValidacion[] = [];
 ...
 */
export const validarSeccionEspecialidades = (
  _formData: EmpresaFormConsolidado,
  _config: ConfiguracionValidacion
): ErrorValidacion[] => {
  const errores: ErrorValidacion[] = [];
...
export const validarFormularioCompleto = (
  formData: EmpresaFormConsolidado,
  config: ConfiguracionValidacion = DEFAULT_VALIDATION_CONFIG
): ResultadoValidacion => {
 ...
 */
export const getCampoLabel = (campo: string): string => {
...
export const validarCampo = (
  ...
  const validadores: Record<string, (formData: EmpresaFormConsolidado, config: ConfiguracionValidacion) => ErrorValidacion[]> = {
  ...
  return erroresSeccion.filter((error: ErrorValidacion) => error.campo === campo);
};

/**
 * Determina a qué sección pertenece un campo
 */
export const getSeccionDelCampo = (campo: string): 'basica' | 'contacto' | 'representantes' | 'registro' | 'especialidades' | null => {
  const mapaCampos: Record<string, 'basica' | 'contacto' | 'representantes' | 'registro' | 'especialidades'> = {
    ruc: 'basica',
    razon_social: 'basica',
