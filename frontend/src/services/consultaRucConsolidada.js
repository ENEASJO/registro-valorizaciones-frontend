 ...
 */
function mapearEspecialidadesOSCE(especialidades) {
    ...
    const especialidadesFormulario = new Set();
    especialidades.forEach(especialidad => {
        ...
        if (mapeoEspecialidades[especialidadUpper]) {
            ...
            if (especialidadUpper.includes('EDIFICACION'))
                especialidadesFormulario.add('EDIFICACIONES');
            if (especialidadUpper.includes('CARRETERA'))
                especialidadesFormulario.add('CARRETERAS');
            if (especialidadUpper.includes('SANEAMIENTO'))
                especialidadesFormulario.add('SANEAMIENTO');
            if (especialidadUpper.includes('ELECTRIC'))
                especialidadesFormulario.add('ELECTRICIDAD');
        }
    });
 ...
 */
function seleccionarRepresentantePrincipal(miembros) {
    ...
    for (const cargoPreferido of prioridadCargos) {
        const miembro = miembros.find(m => m.cargo?.toUpperCase().includes(cargoPreferido));
        if (miembro)
            return miembro;
    }
 ...
 */
function extraerUbicacion(direccion) {
    if (!direccion)
        return {};
 ...
 */
function determinarEstadoEmpresa(registro) {
    // Priorizar estado de SUNAT
    if (registro.estado_sunat) {
        if (registro.estado_sunat.includes('ACTIVO'))
            return 'ACTIVO';
        if (registro.estado_sunat.includes('SUSPENDIDO'))
            return 'SUSPENDIDO';
        if (registro.estado_sunat.includes('INACTIVO'))
            return 'INACTIVO';
    }
    // Fallback a estado de OSCE
    if (registro.estado_osce) {
        if (registro.estado_osce.includes('HABILITADO') || registro.estado_osce.includes('EJECUTOR'))
            return 'ACTIVO';
        if (registro.estado_osce.includes('SUSPENDIDO'))
            return 'SUSPENDIDO';
    }
 ...
 */
function transformarDatosConsolidados(datos) {
 ...
 */
function generarAdvertenciasConsolidadas(datos) {
    const advertencias = [];
    ...
    const representantesConflicto = datos.miembros.filter(m => m.fuente === 'AMBOS' &&
 ...
 */
export async function consultarRucConsolidado(ruc) {
 ...
 */
export async function consultarRucConsolidadoParaFormulario(ruc) {
 ...
 */
export async function verificarDisponibilidadConsolidada() {
    try {
        const controller = new AbortController();
...
class CacheConsolidado {
    cache = new Map();
    CACHE_DURATION = 10 * 60 * 1000; // 10 minutos para datos consolidados
    get(ruc) {
 ...
 */
export async function consultarRucConsolidadoConCache(ruc) {
    const rucLimpio = ruc.replace(/\D/g, '');
