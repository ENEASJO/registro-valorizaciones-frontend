...
type Empresa = Tables['empresas']['Row']
type EmpresaInsert = Tables['empresas']['Insert']
type EmpresaUpdate = Tables['empresas']['Update']
type Obra = Tables['obras']['Row']
type ObraInsert = Tables['obras']['Insert']
type ObraUpdate = Tables['obras']['Update']
type Valorizacion = Tables['valorizaciones']['Row']
type ValorizacionInsert = Tables['valorizaciones']['Insert']
type ValorizacionUpdate = Tables['valorizaciones']['Update']

// ==================== EMPRESAS ====================

export const empresasService = {
  // Obtener todas las empresas
  async getAll(tipo?: 'ejecutora' | 'supervisora' | 'consultora'): Promise<Empresa[]> {
    let query = supabase
      .from('empresas')
      .select('*')
  ...
  async getByRuc(ruc: string): Promise<Empresa | null> {
    const { data, error } = await supabase
      .from('empresas')
      .select('*')
      .eq('ruc', ruc)
  ...
  // Crear nueva empresa
  async create(empresa: EmpresaInsert): Promise<Empresa> {
    const { data, error } = await supabase
      .from('empresas')
      .insert(empresa)
      .select()
  ...
  // Actualizar empresa
  async update(id: string, empresa: EmpresaUpdate): Promise<Empresa> {
    const { data, error } = await supabase
      .from('empresas')
  ...
  async search(searchText: string): Promise<Empresa[]> {
    const { data, error } = await supabase
      .from('empresas')
      .select('*')
...
export const obrasService = {
  ...
  async getAll(): Promise<(Obra & { empresa_ejecutora: Empresa; empresa_supervisora: Empresa | null })[]> {
    const { data, error } = await supabase
      .from('obras')
      .select(`
        *,
  ...
  // Obtener obra por ID
  async getById(id: string): Promise<(Obra & { empresa_ejecutora: Empresa; empresa_supervisora: Empresa | null }) | null> {
    const { data, error } = await supabase
      .from('obras')
      .select(`
        *,
  ...
  // Crear nueva obra
  async create(obra: ObraInsert): Promise<Obra> {
    const { data, error } = await supabase
      .from('obras')
      .insert(obra)
      .select()
  ...
  // Actualizar obra
  async update(id: string, obra: ObraUpdate): Promise<Obra> {
    const { data, error } = await supabase
      .from('obras')
...
export const valorizacionesService = {
  ...
  async getAll(): Promise<(Valorizacion & { obra: Obra })[]> {
    const { data, error } = await supabase
      .from('valorizaciones')
      .select(`
        *,
  ...
  async getByObra(obraId: string): Promise<Valorizacion[]> {
    const { data, error } = await supabase
      .from('valorizaciones')
      .select('*')
      .eq('obra_id', obraId)
  ...
  async getById(id: string): Promise<(Valorizacion & { obra: Obra }) | null> {
    const { data, error } = await supabase
      .from('valorizaciones')
      .select(`
        *,
  ...
  async create(valorizacion: ValorizacionInsert): Promise<Valorizacion> {
    const { data, error } = await supabase
      .from('valorizaciones')
      .insert(valorizacion)
      .select()
  ...
  async update(id: string, valorizacion: ValorizacionUpdate): Promise<Valorizacion> {
    const { data, error } = await supabase
      .from('valorizaciones')
