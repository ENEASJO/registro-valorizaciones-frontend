...
export const useValorizaciones = () => {
    ...
    // FUNCIONES CRUD
    // =================================================================
    const cargarValorizaciones = useCallback(async (filtros) => {
        setLoading(true);
        setError(null);
        try {
            ...
            if (filtros?.obra_id)
                params.append('obra_id', filtros.obra_id.toString());
            if (filtros?.estado)
            ...
            if (filtros?.tipo_valorizacion)
                params.append('tipo_valorizacion', filtros.tipo_valorizacion);
            if (filtros?.solo_con_atraso)
                params.append('solo_con_atraso', 'true');
    ...
    const crearValorizacion = useCallback(async (data) => {
        setLoading(true);
        setError(null);
        try {
            // Validaciones
    ...
    const actualizarValorizacion = useCallback(async (id, data) => {
        setLoading(true);
        setError(null);
        try {
            if (!id) {
            ...
            if (recalcular) {
                datosFinales = calcularTotales(datosCompletos);
            }
            ...
            // Actualizar estado local
            setValorizaciones(prev => prev.map(v => v.id === id ? valorizacionActualizada : v));
    ...
    const eliminarValorizacion = useCallback(async (id) => {
        setLoading(true);
        setError(null);
        try {
            if (!id) {
    ...
    const obtenerValorizacionPorId = useCallback(async (id) => {
        setLoading(true);
        setError(null);
        try {
            if (!id) {
    ...
    // =================================================================
    const calcularTotalesValorizacion = useCallback((data) => {
        return calcularTotales(data);
    }, []);
    const validarPeriodoValorizacion = useCallback((periodo) => {
    ...
    const estadisticasLocales = useMemo(() => {
        ...
        const conFechaAprobacion = valorizaciones.filter(v => v.fecha_aprobacion && v.fecha_presentacion);
        let promedioDias = 0;
    ...
    return {
        ...
        validarFechas: (inicio, fin) => validarFechas(inicio, fin),
        ...
        cambiarEstadoValorizacion: async (id, nuevoEstado) => {
            return await actualizarValorizacion(id, { estado_valorizacion: nuevoEstado });
        },
        formatearMoneda: (monto) => new Intl.NumberFormat('es-PE', {
            style: 'currency',
            currency: 'PEN',
        ...
        cargarPartidasPorObra: async (obraId) => {
        ...
        validarValorizacion: (form) => {
            // Validación básica
            if (!form.obra_id)
                return { valido: false, errores: ['Obra requerida'] };
            if (!form.periodo)
                return { valido: false, errores: ['Período requerido'] };
            if (!form.fecha_inicio || !form.fecha_fin)
                return { valido: false, errores: ['Fechas requeridas'] };
            if (!validarFechas(form.fecha_inicio, form.fecha_fin))
                return { valido: false, errores: ['Fechas inválidas'] };
