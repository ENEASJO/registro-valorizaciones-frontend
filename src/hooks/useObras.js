...
const mapearObraResponse = (response) => ({
    id: response.id,
    ...
    tipo_obra: response.tipo_obra || 'OTROS',
    modalidad_ejecucion: response.modalidad_ejecucion || 'CONTRATA',
    sistema_contratacion: response.sistema_contratacion || 'SUMA_ALZADA',
    estado: response.estado_obra || 'REGISTRADA',
...
const mapearObraFormAPayload = (form) => ({
...
const realizarPeticionHTTP = async (url, options = {}) => {
...
export const useObras = () => {
    ...
    const cargarObras = useCallback(async (filtros) => {
        setLoading(true);
        setError(null);
        try {
    ...
    const crearObra = useCallback(async (params) => {
        setLoading(true);
        setError(null);
        try {
    ...
    // Actualizar obra
    const actualizarObra = useCallback(async (id, obraData) => {
        setLoading(true);
        setError(null);
        try {
            // Crear payload parcial con solo los campos que se van a actualizar
            const payload = {};
            if (obraData.numero_contrato !== undefined)
                payload.codigo = obraData.numero_contrato;
            if (obraData.nombre !== undefined)
                payload.nombre = obraData.nombre;
            if (obraData.descripcion !== undefined)
                payload.descripcion = obraData.descripcion;
            if (obraData.entidad_ejecutora_id !== undefined)
                payload.empresa_id = obraData.entidad_ejecutora_id;
            if (obraData.ubicacion !== undefined)
                payload.ubicacion = obraData.ubicacion;
            if (obraData.distrito !== undefined)
                payload.distrito = obraData.distrito;
            if (obraData.provincia !== undefined)
                payload.provincia = obraData.provincia;
            if (obraData.departamento !== undefined)
                payload.departamento = obraData.departamento;
            if (obraData.modalidad_ejecucion !== undefined)
                payload.modalidad_ejecucion = obraData.modalidad_ejecucion;
            if (obraData.sistema_contratacion !== undefined)
                payload.sistema_contratacion = obraData.sistema_contratacion;
            if (obraData.tipo_obra !== undefined)
                payload.tipo_obra = obraData.tipo_obra;
            if (obraData.monto_ejecucion !== undefined)
                payload.monto_contractual = obraData.monto_ejecucion;
            if (obraData.monto_supervision !== undefined)
                payload.monto_adicionales = obraData.monto_supervision;
            if (obraData.fecha_inicio !== undefined)
                payload.fecha_inicio = obraData.fecha_inicio;
            if (obraData.fecha_termino !== undefined)
                payload.fecha_fin_contractual = obraData.fecha_termino;
            if (obraData.plazo_ejecucion_dias !== undefined)
                payload.plazo_contractual = obraData.plazo_ejecucion_dias;
    ...
    const obtenerObraPorId = useCallback(async (id) => {
        setLoading(true);
        setError(null);
        try {
...
export const useProfesionales = () => {
    ...
    const cargarProfesionalesPorObra = useCallback(async (obraId) => {
        setLoading(true);
        setError(null);
        try {
    ...
    const agregarProfesional = useCallback(async (obraId, profesional) => {
        setLoading(true);
        setError(null);
        try {
            // TODO: Implementar cuando estén disponibles los endpoints de profesionales
    ...
    const verificarDisponibilidad = useCallback(async (nombreCompleto, fechaInicio, fechaFin, obraIdExcluir) => {
        // TODO: Implementar cuando estén disponibles los endpoints de profesionales
...
export const useValidacionesObra = () => {
    const validarNumeroContrato = useCallback((numeroContrato) => {
        if (!numeroContrato) {
    ...
    const validarMontos = useCallback((montoEjecucion, montoSupervision) => {
        const errores = [];
        if (montoEjecucion <= 0) {
    ...
    const validarFechas = useCallback((fechaInicio, plazoDias, fechaTermino) => {
        const errores = [];
    ...
    const validarEntidadesContratistas = useCallback((entidadEjecutoraId, entidadSupervisoraId) => {
        const errores = [];
    ...
    const validarPorcentajeProfesional = useCallback((porcentaje) => {
        const errores = [];
    ...
    const validarObraForm = useCallback((form) => {
        ...
        // Validar entidades
        const erroresEntidades = validarEntidadesContratistas(form.entidad_ejecutora_id, form.entidad_supervisora_id);
...
export const useValorizaciones = () => {
    ...
    const [loading, setLoading] = useState(false);
    const [error, setError] = useState(null);
    const cargarValorizacionesPorObra = useCallback(async (obraId) => {
        setLoading(true);
        setError(null);
        try {
