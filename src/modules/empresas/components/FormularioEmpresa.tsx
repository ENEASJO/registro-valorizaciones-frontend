...
const FormularioEmpresa = ({ 
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // En modo manual, permitir guardar sin representantes (se pueden agregar manualmente después)
    if (!modoManual && isPersonaJuridica(formData.ruc) && formData.representantes.length === 0) {
    ...
    try {
      ...
      const apiData = {
        ...
        fuentes_consultadas: modoManual ? ['MANUAL'] : (formData.fuentes_consultadas || []),
        fuente_datos: modoManual ? 'MANUAL' : 'SCRAPING',
  ...
  const ManualModeBanner = () => (
    <motion.div
      initial={{ opacity: 0, y: -20 }}
      animate={{ opacity: 1, y: 0 }}
      className="rounded-xl p-6 mb-6 bg-gradient-to-r from-orange-50 to-red-50 border-2 border-orange-300"
    >
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1">
          ...
          <p className="text-orange-700 text-sm mb-3">
            Complete los campos manualmente. Los datos no fueron obtenidos automáticamente del scraping.
          </p>
          <div className="bg-white rounded-lg p-3 border border-orange-200">
            <ul className="text-sm text-orange-800 space-y-1">
              <li>✓ Complete la Razón Social manualmente</li>
              <li>✓ Agregue información de contacto</li>
              <li>✓ Para personas jurídicas, puede agregar representantes después</li>
            </ul>
          </div>
        </div>
      </div>
    ...
    <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50 p-4">
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="p-8 space-y-8 overflow-y-auto max-h-[calc(95vh-280px)]">
          {/* Banner de éxito o modo manual */}
          <AnimatePresence>
            {modoManual ? <ManualModeBanner /> : datosObtenidos && <ConsolidationBanner />}
          </AnimatePresence>
          {/* Error Alert */}
          <AnimatePresence>
            {error && (
          ...
          <div className="space-y-6">
            ...
            <div className="flex justify-center gap-4 flex-wrap">
              <button
                ...
                disabled={consultando || !formData.ruc || modoManual}
              ...
              {/* Botón de entrada manual - aparece si hay error o si el usuario lo activa */}
              {(error || modoManual) && !consultando && formData.ruc && (
                <button
                  type="button"
                  onClick={() => {
                    setModoManual(true);
                  ...
                  className="px-12 py-4 bg-gradient-to-r from-orange-500 to-red-500 text-white rounded-xl hover:from-orange-600 hover:to-red-600 flex items-center gap-3 font-bold shadow-lg hover:shadow-xl transition-all text-lg"
                >
                  <FileText className="w-6 h-6" />
                  {modoManual ? 'MODO MANUAL ACTIVO' : 'CONTINUAR CON ENTRADA MANUAL'}
                </button>
              )}
            </div>
          </div>
              ...
              <span className="flex items-center gap-2">
                {modoManual ? (
                  <>
                    ...
                    <span className="text-orange-600">Modo manual - Complete los campos requeridos</span>
                  </>
                ) : (
                  <>
