...
export class GeneradorPDF {
     ...
     */
    static construirContenidoValorizacion(datos, configuracion) {
        let contenido = '';
        // Portada
        if (configuracion.incluirPortada) {
            contenido += this.generarPortada('REPORTE DE VALORIZACIÓN MENSUAL', datos.obra.nombre, datos.valorizacion.periodo);
        }
        // Índice
     ...
     */
    static construirContenidoAvance(datos, configuracion) {
        let contenido = '';
        if (configuracion.incluirPortada) {
            contenido += this.generarPortada('REPORTE DE AVANCE DE OBRA', datos.obra.nombre, `${datos.obra.diasTranscurridos} días de ejecución`);
        }
        if (configuracion.incluirIndice) {
            contenido += this.generarIndice([
                'Información General de la Obra',
     ...
     */
    static construirContenidoFinanciero(datos, configuracion) {
        let contenido = '';
        if (configuracion.incluirPortada) {
            contenido += this.generarPortada('REPORTE FINANCIERO CONSOLIDADO', 'Sistema de Valorizaciones', 'Análisis de Inversión y Liquidez');
        }
        if (configuracion.incluirIndice) {
            contenido += this.generarIndice([
                'Resumen Ejecutivo',
     ...
     */
    static construirContenidoContractual(datos, configuracion) {
        let contenido = '';
        if (configuracion.incluirPortada) {
            contenido += this.generarPortada('REPORTE DE CONTROL CONTRACTUAL', 'Sistema de Valorizaciones', 'Control y Seguimiento de Contratos');
        }
     ...
     */
    static construirContenidoGerencial(datos, configuracion) {
        let contenido = '';
        if (configuracion.incluirPortada) {
            contenido += this.generarPortada('REPORTE GERENCIAL EJECUTIVO', 'Sistema de Valorizaciones', 'Dashboard de Control y Gestión');
        }
    ...
    // =================================================================
    static generarPortada(titulo, subtitulo, periodo) {
        return `
    PORTADA:
    ========
    ${titulo}
    ...
    static generarIndice(secciones) {
        let indice = '\nÍNDICE:\n========\n';
    ...
    static generarSeccionObra(obra) {
        return `
    ...
    static generarSeccionContratista(contratista) {
        return `
    ...
    static generarSeccionSupervisor(supervisor) {
        return `
    ...
    static generarSeccionValorizacion(valorizacion) {
        return `
    ...
    static generarTablaPartidas(partidas) {
        let tabla = `
    ...
    static generarSeccionFirmas(firmas) {
        return `
    ...
    static generarSeccionAvance(avance) {
        return `
    ESTADO ACTUAL DE AVANCE
    =======================
    ...
    static generarGraficoCurvaS(cronograma) {
        return `
    [GRÁFICO: Curva S - Cronograma de Avance]
    Datos del cronograma incluidos...
    `;
    }
    static generarTablaValorizaciones(valorizaciones) {
        let tabla = `
    ...
    static generarSeccionAlertas(alertas) {
        let seccion = `
    ALERTAS Y OBSERVACIONES
    =======================
    `;
    ...
    static generarSeccionRecomendaciones(recomendaciones) {
        let seccion = `
    RECOMENDACIONES
    ===============
    `;
    ...
    static generarResumenEjecutivoFinanciero(resumen) {
        return `
    ...
    static generarGraficoFlujoEfectivo(flujo) {
        return `
    [GRÁFICO: Flujo de Efectivo Mensual]
    Datos del flujo de efectivo incluidos...
    `;
    }
    static generarTablaEstadoPagos(estadoPagos) {
        let tabla = `
    ...
    static generarSeccionDeducciones(deducciones) {
        return `
    ANÁLISIS DE DEDUCCIONES
    =======================
    ...
    static generarSeccionProyecciones(proyecciones) {
        return `
    PROYECCIONES FINANCIERAS
    ========================
    ...
    static generarIndicadoresFinancieros(indicadores) {
        return `
    ...
    static generarTablaContratosVigentes(contratos) {
        let tabla = `
    CONTRATOS VIGENTES
    ==================
    `;
    ...
    static generarSeccionVencimientos(vencimientos) {
        let seccion = `
    ...
    static generarTablaPenalidades(penalidades) {
    ...
    static generarTablaAdicionalesDeductivos(adicionales) {
        let tabla = `
    ...
    static generarTablaGarantias(garantias) {
        let tabla = `
    GARANTÍAS VIGENTES
    ==================
    `;
    ...
    static generarKPIsPrincipales(kpis) {
        return `
    ...
    static generarGraficoDistribucionInversion(distribucion) {
        return `
    [GRÁFICO: Distribución de Inversión por Tipo de Obra]
    Datos de distribución incluidos...
    `;
    }
    static generarRankingContratistas(ranking) {
        let tabla = `
    ...
    static generarAnalisisRiesgos(riesgos) {
        let seccion = `
    ANÁLISIS DE RIESGOS
    ===================
    `;
    ...
    static generarTendenciasPorRegion(tendencias) {
        let tabla = `
    ...
    static generarProyeccionesEstrategicas(proyecciones) {
        return `
    ...
    ${proyecciones.accionesCorrectivas.map((accion, index) => `${index + 1}. ${accion}`).join('\n')}
    OPORTUNIDADES DE MEJORA:
    ${proyecciones.oportunidadesMejora.map((oportunidad, index) => `${index + 1}. ${oportunidad}`).join('\n')}
    `;
    }
}
